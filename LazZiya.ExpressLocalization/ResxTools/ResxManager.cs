...
namespace LazZiya.ExpressLocalization.ResxTools
{
    ...
    public class ResxManager
    {
        ...
        public ResxManager(Type TResxFileType, string resxFolderUri ="", string culture = "")
        {
            TargetResourceFile = string.IsNullOrWhiteSpace(culture)
                ? Path.Combine(resxFolderUri, $"{TResxFileType.Name}.resx")
                : Path.Combine(resxFolderUri, $"{TResxFileType.Name}.{culture}.resx");
        ...
        public async Task<bool> AddAsync(ResxElement element, bool overWriteExistingKeys = false)
        {
            ...
            if (elmnt != null && overWriteExistingKeys == true)
            {
                try
                {
                    _xd.Root.Elements("data").FirstOrDefault(x => x == elmnt).ReplaceWith(element.XElement);
                    tsk.SetResult(true);
                }
                catch (Exception e)
                {
